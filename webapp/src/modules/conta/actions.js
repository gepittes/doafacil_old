...
export const cadastrarConta = ({commit}, conta) => {
    return axios.post('http://localhost/v1/conta', conta)
        .then((response) => {
            const data = response.data;
            commit(types.SET_CONTA, data.data)
        });
...
export const atualizarConta = ({commit}, conta) => {
    return axios.patch('http://localhost/v1/conta/' + conta.usuario_id, conta)
        .then(() => {
